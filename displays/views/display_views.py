...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def list_display_filters_view(request, screen_id: int):
    institution = request.user.institution
    try:
        screen = display_service.get_display_screen_instance_or_404(screen_id, institution)
        filters = display_service.list_display_filters(screen)
        return BaseResponse.success(
            message=SuccessCodes.DISPLAY_FILTER_LISTED["message"],
            code=SuccessCodes.DISPLAY_FILTER_LISTED["code"],
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def create_display_filter_view(request, screen_id: int):
    institution = request.user.institution
    try:
        screen = display_service.get_display_screen_instance_or_404(screen_id, institution)
        created = display_service.create_display_filter(screen, request.data)
        return BaseResponse.success(
            message=SuccessCodes.DISPLAY_FILTER_CREATED["message"],
            code=SuccessCodes.DISPLAY_FILTER_CREATED["code"],
            data={"filter": created},
            status_code=status.HTTP_201_CREATED,
        )
    except CustomValidationError as exc:
        ...
        return BaseResponse.error(
            message=ErrorCodes.DISPLAY_FILTER_CREATION_FAILED["message"],
            code=ErrorCodes.DISPLAY_FILTER_CREATION_FAILED["code"],
            status_code=ErrorCodes.DISPLAY_FILTER_CREATION_FAILED["status_code"],
            errors=ErrorCodes.DISPLAY_FILTER_CREATION_FAILED["errors"],
        )


@api_view(["PUT"])
@permission_classes([IsAuthenticated])
def update_display_filter_view(request, filter_id: int):
    institution = request.user.institution
    try:
        filter_instance = display_service.get_display_filter_instance_or_404(filter_id, institution)
        updated = display_service.update_display_filter(filter_instance, request.data)
        return BaseResponse.success(
            message=SuccessCodes.DISPLAY_FILTER_UPDATED["message"],
            code=SuccessCodes.DISPLAY_FILTER_UPDATED["code"],
            data={"filter": updated},
        )
    except CustomValidationError as exc:
        return BaseResponse.error(
        ...
        return BaseResponse.error(
            message=ErrorCodes.DISPLAY_FILTER_UPDATE_FAILED["message"],
            code=ErrorCodes.DISPLAY_FILTER_UPDATE_FAILED["code"],
            status_code=ErrorCodes.DISPLAY_FILTER_UPDATE_FAILED["status_code"],
            errors=ErrorCodes.DISPLAY_FILTER_UPDATE_FAILED["errors"],
        )


@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
def delete_display_filter_view(request, filter_id: int):
    institution = request.user.institution
    try:
        filter_instance = display_service.get_display_filter_instance_or_404(filter_id, institution)
        display_service.delete_display_filter(filter_instance)
        return BaseResponse.success(
            message=SuccessCodes.DISPLAY_FILTER_DELETED["message"],
            code=SuccessCodes.DISPLAY_FILTER_DELETED["code"],
        )
    except CustomValidationError as exc:
        return BaseResponse.error(
            message=exc.detail["message"],
        ...
        return BaseResponse.error(
            message=ErrorCodes.DISPLAY_FILTER_DELETION_FAILED["message"],
            code=ErrorCodes.DISPLAY_FILTER_DELETION_FAILED["code"],
            status_code=ErrorCodes.DISPLAY_FILTER_DELETION_FAILED["status_code"],
            errors=ErrorCodes.DISPLAY_FILTER_DELETION_FAILED["errors"],
        )
